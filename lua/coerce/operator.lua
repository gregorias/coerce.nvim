...
---
--- This function sets the operatorfunc option to the provided callback and immediately triggers the
--- operator.
...
M.operator_cb = function(user_operator_cb)
	...
	-- 'i' to insert. If we the user has typed "gUe" for uppercasing till end of the word and `gU`
	-- calls `operator_cb`, then `g@` needs to be inserted before the 'e'.
